...
总结来说就是，我们本来可以遍历所有路径，但是计算复杂度为指数。所以我们利用求最大值时的分配律，将求和是转化为线性计算复杂度。在具体算法中，考虑一个特定的时刻$$n$$以及在那个时刻的一个特定的状态$$k$$，即$$z_nk$$，前一个时刻会有$$K$$条路径通往$$z_nk$$，但是只需要保留当前具有最高概率的特定的路径即可。在时刻$$n$$有$$K$$个状态，因此我们需要跟踪$$K$$个这样的路径。当我们到达最后的时刻N时，我们还是只有$$K$$个状态对应的$$K$$条最优路径，我们选择概率最高的即可。然后反向跟踪这条路径的前$$N-1$$步，就可以得到最有可能的状态序列。
