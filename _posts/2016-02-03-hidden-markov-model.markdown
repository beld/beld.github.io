...
总的来说，隐马尔可夫模型有三种应用：  
第一，给定一个观测序列$$x_1,x_2,x_3...$$，假定模型参数已知$$\theta=\{\pi,A,\psi\}$$，求给定的观测序列在此模型下实际发生的概率，比如数据的似然概率$$p(X|\theta)$$。这样如果我们有多个模型，就可以选择概率最高的做为最有可能的模型。似然概率可以通过前向算法计算。这就是一个有监督学习问题，可以看作是推断步骤。在得到数据似然的基础上，我们可以解决两种问题，滤波（Filtering）和平滑（Smoothing)：  
滤波是计算$$p(z_t|x_{1:t})$$，新估计的状态概率只根据之前的观测结果，可以通过前向算法计算。  
平滑是计算$$p(z_t|x_{1:T})$$，状态概率是基于所有的观测，包括未来的，可以通过向前向后算法计算。

第二，给定一个观测序列$$x_1,x_2,x_3...$$，假定模型参数已知$$\theta =\{\pi,A,\psi\}$$，求最佳状态序列$$z_1,z_2,z_3...$$，可以通过维特比算法计算。

第三，给定一个观测序列$$x_1,x_2,x_3...$$，求最佳模型参数$$\theta =\{\pi,A,\psi\}$$。这通常是最难的。可以通过最大期望EM算法计算或者Baum-Welch算法。

#### HMM的最大似然
如果我们观测到一个数据集$$X = \{x_1,...,x_N\}$$，那么我们可以使用最大似然法确定HMM的参数。似然函数通过对联合概率分布中的潜在变量进行求和的方式得到，即
...
分母$$p(X)$$隐式的以HMM的参数$$θ^{old}$$为条件，因此表示似然函数。使用条件独立性质，$$z_n$$阻隔了$$x_1,...,x_n$$与$$x_{n+1},...,x_N$$，所以有
...
维特比算法在Bishop 的prml中记号比较难懂，其实很简单。通过向前向后算法，在给定观测序列下，我们可以推断某个隐含状态，但是不能推出最有可能的隐含状态序列。在隐马尔可夫模型的许多应用中，潜在变量有许多有意义的直观意义，因此对于给定的观测序列，我们常常感兴趣的是寻找概率最高的隐含状态序列。例如，在语音识别中，对于一个给定的声音观测序列，我们可能希望找到概率最大的音素序列。由于隐马尔可夫模型的图是一棵有向树，因此这个问题可以使用最大加和算法精确地求解。回忆一下在图模型中，寻找潜在变量的概率最高的序列与寻找分别概率最高的状态的集合是不相同的。
...
与向前向后算法类似，我们继续推导递归关系，其中利用了求最大值的分配律

$$
\begin{eqnarray}
\mu(z_n) &=& \max_{z_1,...,z_{n-1}} p(x_1,...,x_n,z_1,...,z_n) \\
&=& \max_{z_1,...,z_{n-1}} p(x_1,...,x_{n-1},z_1,...,z_{n-1}) p(x_n|z_k) p(z_n|z_{n-1})  \\
&=& \max_{z_{n-1}} p(x_n|z_k) p(z_n|z_{n-1}) \max_{z_1,...,z_{n-2}} p(x_1,...,x_{n-1},z_1,...,z_{n-1})  \\
\end{eqnarray}$$

最后得到
<center>$$\mu(z_n) = \max_{z_{n-1}} p(x_n|z_k) p(z_n|z_{n-1}) $$</center>
但是，我们本来要求的是最有可能的序列，而上面的式子求的是最大联合概率。别担心，我们记下来最大概率对应的隐含状态就可以。

总结来说就是，我们本来可以遍历所有路径，但是计算复杂度为指数。所以我们利用求最大值时的分配律，将其转化为线性计算复杂度。在具体算法中，考虑一个特定的时刻$$n$$以及在那个时刻的一个特定的状态$$k$$，即$$z_{nk}$$，前一个时刻会有$$K$$条路径通往$$z_{nk}$$，但是只需要保留当前具有最高概率的一条的路径即可。在时刻$$n$$有$$K$$个状态，因此我们需要跟踪$$K$$个这样的路径。当我们到达最后的时刻N时，我们还是只有$$K$$个状态对应的$$K$$条最优路径，我们选择概率最高的即可。然后反向跟踪这条路径的前$$N-1$$步，就可以得到最有可能的状态序列。
