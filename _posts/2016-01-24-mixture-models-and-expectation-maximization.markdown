...
#### EM的另一种观点
EM算法的目标是找到具有潜在变量的模型的最大似然解。将所有模型参数的集合被记作$$θ$$，因此对数似然函数为
...
在E步骤中，我们使用当前的参数值$$θ^{old}$$寻找潜在变量的后验概率分布$$p(Z|X,θ^{old})$$。然后，我们使用这个后验概率分布计算完整数据对数似然函数对于一般的参数值$$θ$$的期望。这个期望被记作$$Q(θ,θ^{old})$$，由
...
1. 选择参数$$θ^{old}$$的一个初始设置。
2. E步骤。计算$$p(Z|X,θ^{old})$$。
3. M步骤。计算$$θ^{new}$$，由
...
##### 完整数据的高斯混合模型
现在考虑对完整数据$${X,Z}$$进行最大化。似然函数的形式为
<center>$$p(X,Z|\mu,\Sigma,\pi) = \prod\limits_{n=1}^N\prod\limits_{k=1}^K\pi_k^{z_{nk}}\mathcal{N}(x_n|\mu_k,\Sigma^k)^{z_{nk}} $$</center>
其中$$z_{nk}$$表示$$z_n$$的第$$k$$个分量。取对数，得到
<center>$$\ln p(X,Z|\mu,\Sigma,\pi) = \sum\limits_{n=1}^N\sum\limits_{k=1}^Kz_{nk}\{\ln\pi_k + \ln\mathcal{N}(x_n|\mu_k,\Sigma_k)\} $$</center>
与不完整数据的对数似然函数进行对比，对数运算现在直接作用于高斯分布上，而高斯分布本身是指数族分布的一个成员。这种方法产生了最大似然问题的一个简单得多的解。由于$$z_n$$是一个$$K$$维向量，并且只有一个元素等于$$1$$，其他所有元素均为$$0$$，因此完整数据的对数似然函数仅仅是$$K$$个独立的贡献的和，每个混合分量都有一个贡献。于是关于均值或协方差的最大化与单一高斯分布的情形完全相同，唯一的区别是它只涉及到被“分配”到那个分量的数据点的子集。对于关于混合系数的最大化问题，与之前一样，可以使用拉格朗日乘数法进行优化。

因此我们看到，完整数据的对数似然函数可以用一种简单的方法求出最大值的解析解。然而，在实际应用中，我们并没有潜在变量的值，因此，与之前的讨论一样，我们考虑完整数据对数似然函数关于潜在变量后验概率分布的期望。这个后验分布的形式为
<center>$$p(Z,X,\mu,\Sigma,\pi) \propto \prod\limits_{n=1}^N\prod\limits_{k=1}^K[\pi_k\mathcal{N}(x_n|\mu_k,\Sigma_k)]^{z_{nk}} $$</center>
然后之前推导的高斯混合模型的EM算法完全相同。

#### 证明EM算法最大化了似然函数
期望最大化算法，或EM算法，是寻找具有潜在变量的概率模型的最大似然解的一种通用的方法。之前在讨论高斯混合模型时启发式地推导出的EM算法，现在我们来证明EM算法确实最大化了似然函数，也为之后变分推断框架推导奠定基础。

考虑一个概率模型，其中我们将所有的观测变量联合起来记作$$X$$，将所有的隐含变量记作$$Z$$。联合概率分布$$p(X,Z|θ)$$由一组参数控制，记作$$θ$$。我们的目标是最大化似然函数
<center>$$p(X|\theta) = \sum\limits_Zp(X,Z|\theta)$$</center>
这里，我们假设Z是离散的，不过当Z是连续变量或者离散变量与连续变量的组合时，方法是完全相同的，只需把求和换成适当的积分即可。
我们假设直接最优化$$p(X|θ)$$比较困难，但是最优化完整数据似然函数$$p(X,Z|θ)$$就容易得多。接下来，我们引入一个定义在潜在变量上的分布$$q(Z)$$。我们观察到，对于任意的$$q(Z)$$下面的分解成立
<center>$$\ln p(X|\theta) = L(q,\theta) + KL(q \Vert p) $$</center>
其中，我们定义了
<center>$$L(q,\theta) = \sum\limits_Zq(Z)\ln\left\{\frac{p(X,Z|\theta)}{q(Z)}\right\} $$</center>
<center>$$KL(q\Vert p) = - \sum\limits_Zq(Z)\ln\left\{\frac{p(Z|X,\theta)}{q(Z)}\right\} $$</center>
注意，$$L(q,θ)$$是概率分布$$q(Z)$$的一个泛函，并且是参数$$θ$$的一个函数。推导过程略，其中用到了归一化的概率分布$$q(Z)$$的积分等于$$1$$的事实。

$$KL(q∥p)$$是$$q(Z)$$和后验概率分布$$p(Z|X,θ)$$之间的$$KL$$散度（Kullback-Leibler divergence）。$$KL$$散度满足$$KL(q∥p)≥0$$，当且仅当$$q(Z)=p(Z|X,θ)$$时等号成立。因此，$$L(q,θ)$$是$$lnp(X|θ)$$的一个下界。
![](https://mqshen.gitbooks.io/prml/content/Chapter9/images/decomposition.png)
EM算法是一个两阶段的迭代优化算法，用于寻找最大似然解。假设参数向量的当前值为$$θ^{old}$$。在E步骤中，下界$$L(q,θ^{old})$$关于$$q(Z)$$被最大化，而$$θ^{old}$$保持固定。最大化问题的解很容易看出来。我们注意到$$lnp(X|θ^{old})$$不依赖于$$q(Z)$$，因此$$L(q,θ^{old})$$的最大值出现在Kullback-Leibler散度等于$$0$$的时候，换句话说，最大值出现在$$q(Z)$$与后验概率分布$$p(Z|X,θ^{old}$$相等的时候。此时，下界等于对数似然函数，如图：
![](https://mqshen.gitbooks.io/prml/content/Chapter9/images/e_step.png)
在接下来的M步骤中，分布$$q(Z)$$保持固定，下界$$L(q,θ)$$关于$$θ$$进行最大化，得到了某个新值$$θ^{new}$$。这会使得下界$$L$$增大（除非已经达到了极大值），这会使得对应的对数似然函数增大。由于概率分布$$q$$由旧的参数值确定，并且在M步骤中保持固定，因此它不会等于新的后验概率分布$$p(Z|X,θ^{new})$$，从而$$KL$$散度非零。于是，对数似然函数的增加量大于下界的增加量，如图
![](https://mqshen.gitbooks.io/prml/content/Chapter9/images/m_step.png)
EM算法的E步骤和M步骤都增大了对数似然函数的一个良好定义的下界的值，并且完整的EM循环会使得模型的参数向着使对数似然函数增大的方向进行改变（除非已经达到了一个极大值，此时参数保持不变）。
